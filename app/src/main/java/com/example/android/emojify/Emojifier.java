...
class Emojifier {
    ...
    private static void whichEmoji(Face face) {
        ...
        boolean isLeftEyeClose = face.getIsLeftEyeOpenProbability() < LEFT_EYE_OPEN_THRESHOLD;
        boolean isRightEyeClose = face.getIsRightEyeOpenProbability() < RIGHT_EYE_OPEN_THRESHOLD;
        Emoji emoji;
        if (isSmiling) {
            if (isLeftEyeClose && isRightEyeClose) {
                emoji = Emoji.CLOSED_EYE_SMILING;
            } else if (!isLeftEyeClose && !isRightEyeClose) {
                emoji = Emoji.SMILING;
            } else if (isLeftEyeClose) {
